...
final class DashboardInsightsViewModel {
    ...
    func recalculate(
        ...
        let blockData = blocks.map { TimeBlockData(startDate: $0.startDate, status: $0.status, category: $0.category) }
        ...
        let results = await Task.detached(priority: .userInitiated) {
            ...
            // 2. Filter data
            let filteredBlocks = blockData.filter { $0.startDate >= rangeCutoff }

            // 3. Active days (days with at least one completed block)
            let completedBlocks = blockData.filter { $0.status == .completed }
            ...
            // 8. Weekly Averages
            let eightWeeksBlocks = blockData.filter { $0.startDate >= eightWeeksCutoff }
            ...
            if !allActiveDates.isEmpty {
                ...
                if let lastActive = allActiveDates.last, lastActive >= yesterday {
                    var rollingStreak = 1
